...
INSERT INTO `Doctors` VALUES ('D106101','심사임당','1972-11-28','F','106','과장','010-6101-1972','sinsa@bw.com');
...
#문제 3
SELECT `pat_id`,`pat_name` AS '환자이름',`pat_jomin`,`pat_phone`,`doc_name`,`nur_name`
   FROM `Patients` AS a JOIN `Doctors` AS b ON a.doc_id = b.doc_id
   JOIN `Nurses` AS c ON a.nur_id = c.nur_id;

#문제 4
SELECT `pat_name` AS '환자이름',`doc_name` AS '담당의사명',`treat_contents`AS '진료내용',`chart_contents` AS '처방내용',`treat_datetime` AS '진료날짜'
   FROM `Patients` AS a JOIN `doctors` AS b ON a.doc_id = b.doc_id
   JOIN `treatments` AS c ON a.pat_id = c.pat_id
	JOIN `charts` AS d ON c.treat_no = d.treat_no;

#문제 5
SELECT  c.`treat_no` AS '진료번호' ,`pat_name` AS '환자이름',`doc_name` AS '담당의사명',`treat_contents`AS '진료내용',`chart_contents` AS '처방내용',`treat_datetime` AS '진료날짜'
    FROM `Patients` AS a JOIN `doctors` AS b ON a.doc_id = b.doc_id
    JOIN `treatments` AS c ON a.pat_id = c.pat_id
	JOIN `charts` AS d ON c.treat_no = d.treat_no
...
SELECT * from
    (select * ,
		YEAR(NOW()) - CONCAT('19', SUBSTRING(`pat_jomin`,1 , 2)) AS `age`
    FROM `Patients`) as a
    where `age` >= 30 AND `age` < 40;

SELECT * FROM `Patients`
	WHERE YEAR(NOW()) - CONCAT('19', SUBSTRING(`pat_jomin`,1 , 2))
    BETWEEN 30 AND 40;
...
SELECT 
   ...
   FROM `Patients` AS a JOIN `Nurses` AS b ON a.nur_id = b.nur_id
   WHERE `nur_name` = '김태희';

#문제 10
SELECT b.`nur_id`,`nur_name`,COUNT(*) AS '담당환자수'
   FROM `Patients` AS a JOIN `Nurses` AS b ON a.nur_id = b.nur_id
   GROUP BY b.`nur_id` ORDER BY `담당환자수` DESC LIMIT 1;
